...
namespace BN_MCMC
{
    class Adaptive_NS
    {
        ...
        public Adaptive_NS(List<string> _inputFileInfo, NameValueCollection inputSettings, string outputDir, List<string> methodSettings, List<string> numericalSettings, List<string> graphicalSettings)
        {
        ...
        {
            _Session["InnerWorker_NS"] = bgWorker2;
            ...
            string freqFileName = generalFilePath + @"\n__DIC_NS_" + targetFunction + DataFileName + "_freq_with_" + nNodes.ToString() + "_nodes_with_" + nMaxIteration.ToString() + ".csv";
            string squredDiffFileName = generalFilePath + @"\n__DIC_New_NS_" + targetFunction + DataFileName + "_SQUARED_DIFF_freq_with_" + nNodes.ToString() + "_nodes_with_" + nMaxIteration.ToString() + ".csv";
            string logValueFileName = generalFilePath + @"\n__DIC_New_NS_LogTest_" + targetFunction + DataFileName + "_of_" + nNodes.ToString() + "_" + nMaxIteration.ToString() + "it.csv";
            string log_of_Execution = generalFilePath + @"\n__DIC_New_NS_ExecutionLog_" + targetFunction + DataFileName + "_of_" + nNodes.ToString() + "_" + nMaxIteration.ToString() + "it.csv";
            ...
            #region Step 2. Apply Neighbourhood Sampler with Fixed # of iteration
            ...
            for (; iter < nMaxIteration; iter++)
            {
                ...
                else
                {
                    ...
                    bgWorker2.ReportProgress((int)Math.Floor(test1), "NS: iteration " + iter.ToString() + " Completed");
                }
                ...
                #region [Reduction Steps involved] For each 'Z' (element of) 'N_Y', calculate 'Mu_Z' and F(Z) to ACCEPT/REJECT
                ...
                #region make 'Z' = 'Y' initially
                int[][] Z = new int[nNodes][];
                for (int i = 0; i < nNodes; i++)
                {
                    Z[i] = new int[nNodes];
                    for (int j = 0; j < nNodes; j++)
                ...
                int Z_count = Y_neighbourhoodList.Keys.Count();
                //nZ = Z_count;
                ...
                while (Z_count > 0)
                {
                    ...
                    #region Sample a 'Z_ID' from 'N_Y'
                    int Z_neighbourID = rand.Next(0, Y_totalNumofNeighbours);
                    #endregion

                    #region Generate Z with proper operation AND Update all the lists: A_list, D_list, R_list
                    GenerateNeighbour(ref Z, Y_neighbourhoodList, Z_neighbourID);
                    #endregion

                    #region Handle 'Z' & Find 'N_Z'
                    int Z_totalNumofNeighbours = 0;
                    Dictionary<string, string> Z_neighbourhoodList = new Dictionary<string, string>();

                    /* Insert 'Z' itself in its neighbourhood list, as it is a neighbour of itself */
                    /* Note, first element is this list is the network itself, so the neighbourhoodID '0' (usually chosen Randomely) indicates itself */
                    Z_neighbourhoodList.Add("Nil", "No_op");
                    Z_totalNumofNeighbours++;

                    /* sequential executions of "Find_OperableEdges" function 3times will populate all the neighbourhoods in "Y_neighbourhoodList" list */
                    int num_Z_DeletableEdges = Find_OperableEdges(D_list, ref Z_neighbourhoodList, "Del");
                    int num_Z_AddableEdges = Find_OperableEdges(A_list, ref Z_neighbourhoodList, "Add");
                    int num_Z_ReversableableEdges = Find_OperableEdges(R_list, ref Z_neighbourhoodList, "Rev");

                    /*counting number of neighbourhoods */
                    Z_totalNumofNeighbours += num_Z_DeletableEdges + num_Z_AddableEdges + num_Z_ReversableableEdges;

                    #endregion

                    #region Calculate Mu_z for the neighbours of Z
                    Mu_z = Z_totalNumofNeighbours;
                    #endregion

                    #region Calculate f(Z) and Ratio_Z

                    //pringANetwork(Z_count + rand.Next(), Z);

                    /* will be replaced by some function which calculates f(x)*/
                    if (ch_SampDist.Equals("Uniform"))
                        fz = 1.0F;
                    else
                        fz = Calcuate_Fz(X, X_neighbourhoodList, Y_neighbourID, Z, Y_neighbourhoodList, Z_neighbourID);

                    double ratioZ = (fz - Math.Log(Mu_z)) + (Math.Log(Mu_x) - fx);
                    ratioZ = Math.Exp(ratioZ);

                    #endregion


                    #region Debugging purpose: Trying to catch if a X = Z (i.e. MuX=MuZ and logFx = logFZ), but still the Z isn't accepted (i.e. ratioZ != 1)
                    if (isTwoNetworksSame(X, Z) && ratioZ < 1.0)
                    {
                        int comeHere = 0;
                    }
                    #endregion
                    ...
                    if (ratioZ >= Ux)
                    {
                        if (ch_AcceptRatio.Equals("MH"))
                        {
                            ...
                            if (iter >= nBurnInIteration)
                            {
                                ...
                                SampledNetwork.Add(sX, "S");

                                if (SampledNetwork_Fx[sX] == null)
                                    SampledNetwork_Fx.Add(sX, _global_LogFx.ToString());

                                if (ch_LogPosteriori.Equals("true"))
                                {
                                    int networkID = FindNetworkID(SampledNetwork, sX);
                                    twFx.WriteLine(_global_LogFx.ToString() + "," + networkID.ToString());
                                    ListLogFx.Add(_global_LogFx);
                                }
                            }
                            #endregion
                    ...
                    {
                        #region Revert back Z to Y
                        RevertZbacktoinitialY(ref Z, Y_neighbourhoodList, Z_neighbourID);
                        #endregion

                        #region Remove 'Z' from the Neighbourhood list to prevent to be sampled again
                        string temp_key = Y_neighbourhoodList.Keys.ToArray()[Z_neighbourID];
                        Y_neighbourhoodList.Remove(temp_key);
                        #endregion

                        #region Reduce the Z_count
                        Z_count--;
                        Y_totalNumofNeighbours--;
                        #endregion
            ...
            if (!bgWorker2.CancellationPending)
            {
                ...
                if (ch_Mean_Post_Net.Equals("true"))
                {
                    ...
                    _Session["MeanPostNet_NS"] = MeanPostNetwork;
                }
                else
                    _Session["MeanPostNet_NS"] = null;
                #endregion
                ...
                if (ch_Highest_Freq_Net.Equals("true"))
                {
                    ...
                    _Session["HighestPostNet_NS"] = HighestFreqNetwork;

                }
                else
                    _Session["HighestPostNet_NS"] = null;
                #endregion
                ...
                {
                    twFx.Close();
                    _Session["logFx_NS"] = ListLogFx;
                }
                else
                    _Session["logFx_NS"] = null;
                #endregion

                bgWorker2.ReportProgress(100, "NS: Task completed successfully");
                bgWorker2.ReportProgress(111, "NS: Task completed successfully");
            }
            else
            {
                bgWorker2.ReportProgress(-1, "NS: Operation Cancelled");
            }
            #endregion
        ...
        public void ChangeProgress(object sender, ProgressChangedEventArgs e)
        {
            ...
            progressCollector.StoreCurrentStatusToSession("NS");
        }

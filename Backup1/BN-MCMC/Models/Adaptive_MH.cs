...
namespace BN_MCMC
{
    class Adaptive_MH
    {
        ...
        public Adaptive_MH(List<string> _inputFileInfo, NameValueCollection inputSettings, string outputDir, List<string> methodSettings, List<string> numericalSettings, List<string> graphicalSettings)
        {
        ...
        {
            _Session["InnerWorker_MH"] = bgWorker2;
            ...
            string freqFileName = generalFilePath + @"\n__DIC_MH_" + targetFunction + DataFileName + "_freq_with_" + nNodes.ToString() + "_nodes_with_" + nMaxIteration.ToString() + ".csv";
            string squredDiffFileName = generalFilePath + @"\n__DIC_New_MH_" + targetFunction + DataFileName + "_SQUARED_DIFF_freq_with_" + nNodes.ToString() + "_nodes_with_" + nMaxIteration.ToString() + ".csv";
            string logValueFileName = generalFilePath + @"\n__DIC_New_MH_LogTest_" + targetFunction + DataFileName + "_of_" + nNodes.ToString() + "_" + nMaxIteration.ToString() + "it.csv";
            string log_of_Execution = generalFilePath + @"\n__DIC_New_MH_ExecutionLog_" + targetFunction + DataFileName + "_of_" + nNodes.ToString() + "_" + nMaxIteration.ToString() + "it.csv";
            ...
            for (; iter < nMaxIteration; iter++)
            {
                ...
                else
                {
                    ...
                    bgWorker2.ReportProgress((int)Math.Floor(test1), "MH: iteration " + iter.ToString() + " Completed");
                }
            ...
            if (!bgWorker2.CancellationPending)
            {
                ...
                if (ch_Mean_Post_Net.Equals("true"))
                {
                    ...
                    _Session["MeanPostNet_MH"] = MeanPostNetwork;
                }
                else
                    _Session["MeanPostNet_MH"] = null;
                ...
                if (ch_Highest_Freq_Net.Equals("true"))
                {
                    ...
                    _Session["HighestPostNet_MH"] = HighestFreqNetwork;
                }
                else
                    _Session["HighestPostNet_MH"] = null;
                ...
                {
                    twFx.Close();
                    _Session["logFx_MH"] = ListLogFx;
                }
                else
                    _Session["logFx_MH"] = null;
                #endregion

                bgWorker2.ReportProgress(100, "MH: Task completed successfully");
                bgWorker2.ReportProgress(111, "MH: Task completed successfully");
            }
            else
            {
                bgWorker2.ReportProgress(-1, "MH: Operation Cancelled");
            }
            #endregion
        ...
        public void ChangeProgress(object sender, ProgressChangedEventArgs e)
        {
            ...
            progressCollector.StoreCurrentStatusToSession("MH");
        }

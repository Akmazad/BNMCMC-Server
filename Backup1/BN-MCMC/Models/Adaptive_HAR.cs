...
namespace BN_MCMC
{
    class Adaptive_HAR
    {
        ...
        public System.Web.SessionState.HttpSessionState _Session = null;
        ...
        ulong nMaxIteration;
        ulong stepSize;
        ulong nBurnInIteration;

        /* HnR parameters */
        int _lambda_max = 0;
        int _lambda = 0;
        string option_u = "Simple_Uniform";
        ...
        string ch_InitNet = "";
        string ch_SampDist = "";
        string ch_AcceptRatio = "";


        /* declare numerical parameters */
        string ch_SampledNetDist = "";
        string ch_LogPosteriori = "";
        string ch_SSD = "";
        string ch_MeanPosteriori = "";


        /* declare graphical parameters */
        string ch_LogPost_VS_iter = "";
        string ch_SSD_VS_StepSize = "";
        string ch_TD_VS_ED = "";
        string ch_Mean_Post_Net = "";
        string ch_Highest_Freq_Net = "";
        #endregion

        public Adaptive_HAR(List<string> _inputFileInfo, NameValueCollection inputSettings, string outputDir, List<string> methodSettings, List<string> numericalSettings, List<string> graphicalSettings, string HAR_lambda)
        {
            ...
            nChild_th = Int32.Parse(inputSettings["nMaxOutdegree"]);
            nParent_th = Int32.Parse(inputSettings["nMaxIndegree"]);
            //stepSize = (ulong)Int32.Parse(inputSettings["nSSDstepsize"]);
            InputFile = _inputFileInfo[0];
            DataFileName = _inputFileInfo[1];

            generalFilePath = outputDir;

            /* set Methods settings */
            ch_InitNet = methodSettings[0];
            ch_SampDist = methodSettings[1];
            ch_AcceptRatio = methodSettings[2];
            _lambda_max = Int32.Parse(HAR_lambda);  // only HAR parameter

            /* set Numerical output settings */
            ch_SampledNetDist = numericalSettings[0];
            ch_LogPosteriori = numericalSettings[1];
            ch_SSD = numericalSettings[2];
            ch_MeanPosteriori = numericalSettings[3];

            /* set Graphical output settings */
            ch_LogPost_VS_iter = graphicalSettings[0];
            ch_SSD_VS_StepSize = graphicalSettings[1];
            ch_TD_VS_ED = graphicalSettings[2];
            ch_Mean_Post_Net = graphicalSettings[3];
            ch_Highest_Freq_Net = graphicalSettings[4];
            #endregion
        }

        public void StartAnalysis(System.Web.SessionState.HttpSessionState session)
        {
            _Session = session;
        ...
        {
            _Session["InnerWorker_HAR"] = bgWorker2;
            ...
            string freqFileName = generalFilePath + @"\n__DIC_HAR_" + targetFunction + DataFileName + "_freq_with_" + nNodes.ToString() + "_nodes_with_" + nMaxIteration.ToString() + ".csv";
            string squredDiffFileName = generalFilePath + @"\n__DIC_New_HAR_" + targetFunction + DataFileName + "_SQUARED_DIFF_freq_with_" + nNodes.ToString() + "_nodes_with_" + nMaxIteration.ToString() + ".csv";
            string logValueFileName = generalFilePath + @"\n__DIC_New_HAR_LogTest_" + targetFunction + DataFileName + "_of_" + nNodes.ToString() + "_" + nMaxIteration.ToString() + "it.csv";
            string log_of_Execution = generalFilePath + @"\n__DIC_New_HAR_ExecutionLog_" + targetFunction + DataFileName + "_of_" + nNodes.ToString() + "_" + nMaxIteration.ToString() + "it.csv";

            if (ch_LogPosteriori.Equals("true"))
                twFx = new StreamWriter(logValueFileName);
            ...
            TextReader tr1 = new StreamReader(InputFile);
            DataSet = new int[nNodes][];
            int dat_i_indx = 0;
            while (true)
            {
                string st = tr1.ReadLine();
                if (st == null)
                    break;


                string[] stall = st.Split("\t".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                DataCategories.Add(stall[0].Trim(), stall[1].Trim().Replace("_", ""));

                /* inset the node names int the list */
                nodeNameList.Add(dat_i_indx.ToString(), stall[0].Trim());

                /* load data into a 2D matrix */
                string[] data = stall[2].Trim().Split(",".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                nObservation = data.Count();

                DataSet[dat_i_indx] = new int[nObservation];
                int dat_j_indx = 0;
                foreach (string s in data)
                {
                    DataSet[dat_i_indx][dat_j_indx] = Int32.Parse(s);
                    dat_j_indx++;
                }
                dat_i_indx++;
            }
            ...
            #region Step 2. Apply HnR Sampler with Fixed # of iteration
            ...
            for (; iter < nMaxIteration; iter++)
            {
                ...
                {
                    double test1 = ((double)iter / (double)nMaxIteration) * 100.0;
                    bgWorker2.ReportProgress((int)Math.Floor(test1), "HAR: iteration " + iter.ToString() + " Completed");
                }
                ...
                //NameValueCollection _historyForX_k = new NameValueCollection();
                Dictionary<string, string> _historyForX_k = new Dictionary<string, string>();
                string sX = MakeNetworkString(X);
                _historyForX_k.Add(sX, "X_0");
                ...
                #region option #1: choose 'u' uniformly with some uppper bound
                double upBoundX = (double)(fx / ((double)((nNodes * nNodes - nNodes) / 2.0))); // 235 and 236 lines for f(x)=1
                double Ux1 = GetRandomeNumber(0.0F, upBoundX);
                #endregion

                #region option #2: choose 'u' uniformly from (0,1)
                double Ux2 = rand.NextDouble(); // this is for Target function
                #endregion

                if (option_u.Equals("Simple_Uniform"))
                    Ux = Ux2;
                else if (option_u.Equals("Modified_Uniform"))
                    Ux = Ux1;

                #endregion

                #region Draw lambda uniformly between [1, _lambda_max]
                _lambda = rand.Next(1, _lambda_max + 1);
                #endregion
                ...
                bool _isNetSame = isTwoNetworksSame(X, Y);

                #region Sample X^lambda by selecting X^k iteratively where k = 1,2,.....,lambda
                string Y_edge_info = X_neighbourhoodList.Keys.ToArray()[Y_neighbourID];
                string sY = MakeNetworkString(Y);
                _historyForX_k.Add(sY, Y_edge_info);

                int Mu_Y = 0;
                for (int k = 2; k < _lambda; k++)
                {
                    ...
                    #region Calculate Mu_Y for the neighbours of X and Return
                    Mu_Y = Y_totalNumofNeighbours;
                    #endregion

                    #endregion

                    /* this infinite loop makes sure to sample distinct networks; for which we maintain the history of sampled network in this FOR loop */
                    while (true)
                    {

                        #region Sample Y_ID
                        /* Sample Y from N(X): this random number specifies any integer in the range (0, X_totalNumofNeighbours); Note, +1 indicates the rand function here excludes the last number*/
                        int Y_new_ID = rand.Next(0, Y_totalNumofNeighbours);
                        #endregion

                        #region Make a network from ID and check if it was existing already in the path
                        /* have to fine a new network */
                        if (isNet_in_the_CheckHistory(Y, Y_new_ID, Y_neighbourhoodList, _historyForX_k))
                        {
                            #region Generate Y with proper operation AND Update all the lists: A_list, D_list, R_list
                            GenerateNeighbour(ref Y, Y_neighbourhoodList, Y_new_ID);
                            #endregion

                            #region add the network in the history
                            string sY_new = MakeNetworkString(Y);

                            string new_Y_edge_info = Y_neighbourhoodList.Keys.ToArray()[Y_new_ID];

                            _historyForX_k.Add(sY_new, new_Y_edge_info);
                            #endregion
                ...
                #region Handle 'Y_lambda' & Find 'N_Y_lambda'
                int Y_totalNumofNeighbours_lambda = 0;
                Dictionary<string, string> Y_neighbourhoodList_lambda = new Dictionary<string, string>();

                /* Insert 'Y' itself in its neighbourhood list, as it is a neighbour of itself */
                /* Note, first element is this list is the network itself, so the neighbourhoodID '0' (usually chosen Randomely) indicates itself */
                //Y_neighbourhoodList_lambda.Add("Nil", "No_op");
                //Y_totalNumofNeighbours_lambda++;

                /* sequential executions of "Find_OperableEdges" function 3times will populate all the neighbourhoods in "Y_neighbourhoodList" list */
                int num_Y_DeletableEdges_lambda = Find_OperableEdges(D_list, ref Y_neighbourhoodList_lambda, "Del");
                int num_Y_AddableEdges_lambda = Find_OperableEdges(A_list, ref Y_neighbourhoodList_lambda, "Add");
                int num_Y_ReversableableEdges_lambda = Find_OperableEdges(R_list, ref Y_neighbourhoodList_lambda, "Rev");

                /*counting number of neighbourhoods */
                Y_totalNumofNeighbours_lambda += num_Y_DeletableEdges_lambda + num_Y_AddableEdges_lambda + num_Y_ReversableableEdges_lambda;

                #region Calculate Mu_Y for the neighbours of X and Return
                Mu_Y = Y_totalNumofNeighbours_lambda;
                #endregion

                #endregion

                #region Calculate f(Y_lambda)
                double fy = 0.0;
                if (ch_SampDist.Equals("Uniform"))
                    fy = 1.0;
                else
                    fy = Calcuate_Fy(Y, _historyForX_k);
                #endregion
                ...
                #region Commented
                //if ((fy - fx) >= Math.Log(Ux) && option_acceptance.Equals("Metropolis"))
                //{
                //    #region Accept the sample: copy tempX into X
                //    for (int i = 0; i < nNodes; i++)
                //        for (int j = 0; j < nNodes; j++)
                ...
                //    #endregion
                //}
                //else if ((fx - (Math.Log((double)((nNodes * nNodes - nNodes) / 2.0)))) >= Math.Log(Ux) && option_acceptance.Equals("Modified_Ratio"))
                //{
                //    #region Accept the sample: copy tempX into X
                //    for (int i = 0; i < nNodes; i++)
                //        for (int j = 0; j < nNodes; j++)
            ...
            if (!bgWorker2.CancellationPending)
            {
                ...
                #region Write Frequency file
                if (ch_SampledNetDist.Equals("true"))
                {
                    TextWriter twNew = new StreamWriter(freqFileName);
                    twNew.WriteLine("Frequency,Network");
                    foreach (string key in SampledNetwork.AllKeys)
                    {
                        ...
                        twNew.WriteLine(cnt.ToString() + "," + key);
                    }
                    twNew.Close();
                }
                #endregion

                #region Write SSD file
                //if (ch_SSD.Equals("true"))
                //{
                //    TextWriter twSD = new StreamWriter(squredDiffFileName);
                //    twSD.WriteLine(lineSquaredDiff);
                //    twSD.Close();
                //}
                #endregion

                #region Display True Distribution VS Estimated Distribution (sorted based on Estimated Distribution (from Frequency)
                //if (ch_TD_VS_ED.Equals("true"))
                //{
                //    List_TD_vs_ED = Find_TD_vs_ED_for_all_sampled_Networks();
                //}
                #endregion

                #region Display Mean Posteriori Network
                if (ch_Mean_Post_Net.Equals("true"))
                {
                    #region Initialize arrays
                    int[][] count = new int[nNodes][];

                    for (int i = 0; i < nNodes; i++)
                        count[i] = new int[nNodes];
                    #endregion
                    ...
                    #region Fill count array
                    foreach (string sampleNet in SampledNetwork2.Keys.ToList())
                    {
                        //string[] SampleNetString = ConvertNetStringToArray(sampleNet.Split("|".ToCharArray(), StringSplitOptions.RemoveEmptyEntries));
                        string[] SampleNetString = sampleNet.Replace("\"", "").Split("|".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                        for (int i = 0; i < SampleNetString.Count(); i++)
                        {
                            string[] arr = ConvertNetStringToArray(SampleNetString[i]);
                            for (int j = 0; j < arr.Count(); j++)
                            {
                                if (arr[j].Equals("1"))
                                    count[i][j] += SampledNetwork2[sampleNet];
                            }
                        }
                    }
                    #endregion
                    ...
                    /* create a datatable */
                    MeanPostNetwork = new DataTable("MeanPostNetwork");
                    DataColumn col_source = new DataColumn("Source", typeof(System.String)); MeanPostNetwork.Columns.Add(col_source);
                    DataColumn col_target = new DataColumn("Target", typeof(System.String)); MeanPostNetwork.Columns.Add(col_target);
                    DataColumn col_MeanPosterior = new DataColumn("Mean Posterior", typeof(System.Double)); MeanPostNetwork.Columns.Add(col_MeanPosterior);
                    #endregion

                    #region Fill Posteriori dataTable
                    for (int i = 0; i < nNodes; i++)
                    {
                        for (int j = 0; j < nNodes; j++)
                        {
                            double val = (((double)count[i][j]) / ((double)nMaxIteration));
                            string sourceNode = nodeNameList[i.ToString()];
                            string targetNode = nodeNameList[j.ToString()];

                            /* keep only those edges for which mean posteriori is above the threshold */
                            double th_MeanPosteriori = double.Parse(ch_MeanPosteriori.Split(":".ToCharArray(), StringSplitOptions.RemoveEmptyEntries)[1]);

                            //MeanPostNetwork.Rows.Add(sourceNode, targetNode, val);
                            if (val >= th_MeanPosteriori)
                                MeanPostNetwork.Rows.Add(sourceNode, targetNode, val);
                            //else
                            //    MeanPostNetwork.Rows.Add(sourceNode, targetNode, 0.0);
                        }
                    }
                    #endregion

                    _Session["MeanPostNet_HAR"] = MeanPostNetwork;
                }
                else
                    _Session["MeanPostNet_HAR"] = null;
                #endregion

                #region Display Highest Frequency Network
                if (ch_Highest_Freq_Net.Equals("true"))
                {
                    #region Find the highest frequency network with its frequency
                    string sNet = Find_Network_with_Highest_Frequency();
                    #endregion

                    #region Convert the network from string format to get the edges of highest frequency net

                    #region Create a datatable
                    /* create a datatable */
                    HighestFreqNetwork = new DataTable("HighestFreqNetwork");
                    DataColumn col_source = new DataColumn("Source", typeof(System.String)); HighestFreqNetwork.Columns.Add(col_source);
                    DataColumn col_target = new DataColumn("Target", typeof(System.String)); HighestFreqNetwork.Columns.Add(col_target);
                    #endregion
                    ...
                    for (int i = 0; i < nNodes; i++)
                    {
                        ...
                        for (int j = 0; j < nNodes; j++)
                        {
                            int _edge = Int32.Parse(_cnet[j].ToString());
                            if (_edge == 1)
                            {
                                string sourceNode = nodeNameList[i.ToString()];
                                string targetNode = nodeNameList[j.ToString()];
                                HighestFreqNetwork.Rows.Add(sourceNode, targetNode);
                            }
                    ...
                    _Session["HighestPostNet_HAR"] = HighestFreqNetwork;
                }
                else
                    _Session["HighestPostNet_HAR"] = null;
                #endregion

                #region Write Log Fx file (numerical output
                if (ch_LogPosteriori.Equals("true"))
                {
                    twFx.Close();
                    _Session["logFx_HAR"] = ListLogFx;
                }
                else
                    _Session["logFx_HAR"] = null;
                #endregion

                bgWorker2.ReportProgress(100, "HAR: Task completed successfully");
                bgWorker2.ReportProgress(111, "HAR: Task completed successfully");
            }
            else
            {
                bgWorker2.ReportProgress(-1, "HAR: Operation Cancelled");
            }
            #endregion
        ...
        {
            progressCollector = new ProgressCollector(_Session);
            ...
            progressCollector.StoreCurrentStatusToSession("HAR");
        }
        ...
        public void CompleteLongProcess(object sender, System.ComponentModel.RunWorkerCompletedEventArgs e)
        {
            bgWorker2.Dispose();
            //bgWorker2.Dispose();
            //_Current.Response.Write("<script type='text/javascript' src='Scripts/Progress.js'>LoadResultPage();</script>");
            //_Current = (System.Web.HttpContext)_Session["currentContext"];
            ////_Current.Server.ClearError();
            ////_Current.Response.Headers.Clear();
            ////_Current.Response.Flush();

            ////_Current.Response.Redirect("Results.aspx");
            //_Current.Response.Write("<script type='text/javascript'>");
            //_Current.Response.Write("window.location = 'Results.aspx' </script>");
        }
        private string Find_Network_with_Highest_Frequency()
        {
            Dictionary<string, int> SampledNetwork2 = new Dictionary<string, int>();
        ...
        private double Calcuate_Fy(int[][] _Y, Dictionary<string, string> __historyForX_k)
        {
            /* find list of nodes for which nParents has changed */
            Dictionary<int, List<int>> L_changedNodes = FindNodeswithChangedParents(_Y, __historyForX_k);
        ...
        private bool isTwoNetworksSame(int[][] X, int[][] Y)
        {
            for (int i = 0; i < nNodes; i++)
            {
                for (int j = 0; j < nNodes; j++)
        ...
        private Dictionary<int, List<int>> FindNodeswithChangedParents(int[][] _Y, Dictionary<string, string> __historyForX_k)
        {
            Dictionary<int, List<int>> retList = new Dictionary<int, List<int>>();

            List<int> indices = new List<int>();
            foreach (string key in __historyForX_k.Keys)
            {
                if (__historyForX_k[key].Equals("X_0"))
                    continue;

                string[] temp = __historyForX_k[key].Split("#".ToCharArray(), StringSplitOptions.RemoveEmptyEntries)[0].Split(":".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                int _i = Int32.Parse(temp[0]);
                int _j = Int32.Parse(temp[1]);
                List<int> L_y_i = new List<int>();
                List<int> L_y_j = new List<int>();
                ...
                if (!retList.ContainsKey(_i))
                    retList.Add(_i, L_y_i);
                if (!retList.ContainsKey(_j))
                    retList.Add(_j, L_y_j);
            }

            return retList;
        }

        private bool isNet_in_the_CheckHistory(int[][] tempNet2, int _neighbourID, Dictionary<string, string> _neighbourhoodList, Dictionary<string, string> _historyForX_k)
        {
            ...
            /* if edge != Nil, it means '_neighbourID'='0', means the network itself, so no operation required */
            ...
            string sTemp = MakeNetworkString(tempNet2);
            ...
            if (!_historyForX_k.ContainsKey(sTemp))
                return true;
            else
                return false;
        }

        private double GetRandomeNumber(float v, double upBoundX)
        {
            #region Option 1
            //double val = 0.0;
            //while (true)
            //{
            //    val = (double)rand.NextDouble() * (upBoundX - p) + p;
            //    if (val != 0.0)
            //        break;
            ...
            #region Option 2
            return (double)rand.NextDouble() * (upBoundX - v) + v;
            #endregion
        }
        ...
        private void UseFixedInitialNetwork(ref int[][] net)
        {
            ...
            //   TextReader tr_net = new StreamReader("oN_51_nodes_originalGraph.txt");
            TextReader tr_net = new StreamReader("oN_51_nodes_ges.txt");
        ...
        private void GenerateNeighbour(ref int[][] tempNet, Dictionary<string, string> _neighbourhoodList, int _neighbourID)
        {
            ...
            {
                //if (tempNet[_i][_j] == 0)
                //    //MessageBox.Show("Incorrect Edge to be Deleted! ");
                tempNet[_i][_j] = 0;
            }
            else if (op.Equals("Add"))
            {
                //if (tempNet[_i][_j] == 1)
                //    //MessageBox.Show("Incorrect Edge to be Added! ");
                tempNet[_i][_j] = 1;
            }
            else if (op.Equals("Rev"))
            {
                //if (tempNet[_j][_i] == 1)
                //    //MessageBox.Show("Incorrect Edge to be Reversed! ");
